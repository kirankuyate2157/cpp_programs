...
public:
    ...
    int uniquePathsIII2(vector<vector<int>> &grid)
    {
        int n = grid.size();
        int m = grid[0].size();
        ...
        return dfs(a, b, n, m, grid, ep);
    }

    int dfs(int i, int j, int n, int m, vector<vector<int>> &grid, int ep)
    {
        ...
        grid[i][j] = -1;
        ep--;
        int sum = dfs(i + 1, j, n, m, grid, ep) +
                  dfs(i - 1, j, n, m, grid, ep) +
                  dfs(i, j + 1, n, m, grid, ep) +
                  dfs(i, j - 1, n, m, grid, ep);
        grid[i][j] = 0;
        ep++;
        return sum;
